...
echo "# creating log folder: log_size${size}_${current_date_time}"
mkdir ./experiments/logs
mkdir ./experiments/logs/log_size${size}_${current_date_time}
cd ./src/
echo "# running size $size benchmarks..."
for filename in $prefix; do
# for dir in ../examples/ase_benchmarks_regularized/$size/$prefix*; do
	dir="../examples/ase_benchmarks_regularized/$size/$filename"
	echo "# running benchmark $dir..."
	python ./bmc-synthesizer.py --file $dir > ../experiments/logs/log_size${size}_${current_date_time}/$filename.log --timeout $timeout 2>&1
done